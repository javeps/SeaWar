package com.toosNets.netty.map;
...
public class Bio2GJava {
	...
	public static void g2bean(Class<?> c, String namespace, StrBuilder sb) {
		...
		sb.pn("");
		if (cname.equals("NAttackPoints")) {
			System.out.println("NAttackPoints");
		}
		for (Field field : fs) {
			...
			if (field.getType().equals(List.class)) {
				...
				if (gm.equals("getObject")) {
					sb.pn("        r.${1} = ${2}.parse(map2.getNewMap(${3}));",
							s, t, hs);
				} else if (gm.equals("get")) {
					sb.pn("        r.${1} = (${2})(map2.${3}(${4}));", s, t,
							gm, hs);
				} else {
					sb.pn("        r.${1} = map2.${2}(${3});", s, gm, hs);
				}
	...
	static public void pvService(Method[] methods, StrBuilder sb) {
		// A////////////////////////////////////////////////
		sb.pn("    // //////////////////////////////////////////////");
		sb.pn("    // PV操作 - 逻辑分发");
		sb.pn("    // //////////////////////////////////////////////");
		...
		for (Method m : methods) {
			...
			// 解析参数函数
			sb.pn("    // ${1}", remark);
			sb.pn("    private String __${1}_params(NewMap map2) throws Exception {",
					mname);
