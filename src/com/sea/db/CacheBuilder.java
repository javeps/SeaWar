...
import com.bowlong.sql.builder.jdbc.mysql.JavaType;
import com.bowlong.util.MapEx;
import com.sea.content.AppContext;

public class CacheBuilder extends Toolkit {
	...
	public static String file(String pkg, boolean src, String type,
		...
		if (src)
			path = "src/" + path;
		String cName=PinYin.getShortPinYin(tablename);
	...
	public static void main(String[] args) throws Exception {
		String sql = "SELECT * FROM `user_` LIMIT 1";
		String bpackage = "coc.db";
		String pkg = CacheBuilder.class.getPackage().getName();;
		Connection conn = AppContext.ds().getConnection();
		boolean batch = true;
		boolean sorted = true;
		
		ResultSet rs = SqlEx.executeQuery(conn, sql);
		boolean src = FileEx.exists("src");
		String xml = build(conn, rs, bpackage, pkg, batch, sorted);
		System.out.println(xml);
		String tablename="user_";
		String fn=file(pkg, src, "Cache", tablename+"Cache", "java");
		File f = new File(fn);
	...
	public static String build(Connection conn, ResultSet rs, String pkg, String appContext, boolean batch, boolean sorted) throws Exception {
		...
		sn(sb, "public abstract class %sCache {", tableUEn);
