...
back_file=/root/seawar/dbHour/

back_data_beg=${back_file}dump.rdb.${ymdhms}

back_data_end=${back_data_beg}.tar.gz

back_data_relative=dbHour/dump.rdb.${ymdhms}.tar.gz

origin_data_relative=dbHour/dump.rdb
...
#如:0 */6 * * * sh /root/seawar/RDBHour112.sh
#如:0 */6 * * * sh /root/seawar/RDBHour.sh
#然后ctrl+x 退出编写
#crontab -l 查看定时任务列表